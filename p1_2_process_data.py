...
jdbc_df.selectExpr("CAST(result_id AS STRING) AS key", "to_json(struct(*)) AS value") \
    ...
    .option("topic", "artur_home_athlete_event_results") \
    .save()
...
kafka_streaming_df = spark.readStream \
    ...
    .option("kafka.sasl.jaas.config",
            'org.apache.kafka.common.security.plain.PlainLoginModule required username="admin" password="VawEzo1ikLtrA8Ug8THa";') \
    .option("subscribe", "artur_home_athlete_event_results") \
...
def foreach_batch_function(df, epoch_id):
    ...
    df.selectExpr("CAST(NULL AS STRING) AS key", "to_json(struct(*)) AS value") \
        ...
        .option("kafka.sasl.jaas.config",
            'org.apache.kafka.common.security.plain.PlainLoginModule required username="admin" password="VawEzo1ikLtrA8Ug8THa";') \
        .option("topic", "artur_home_enriched_athlete_avg") \
        .save()
    ...
    df.write.format('jdbc').options(
        ...
        driver='com.mysql.cj.jdbc.Driver',
        dbtable="artur_mykhailiuk_enriched_athlete_avg",
        user=jdbc_user,
